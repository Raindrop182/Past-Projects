...
# %%
CSV_PATH = r"C:/Users/raine/Data/School/MIT/Freshman Year/UROP/CSV/lyze/samples/2-11/alldata-10bins.csv"
OUTPUT_PATH = (
...
def add_month_year_labels(ax,x_axis,y_max,dates):
    ...
    for i, (year, month) in enumerate(zip(years, months)):
        if (year, month) not in seen and month in (1, 7):
            seen.add((year, month))
            ax.text(
                x_axis[i],
                month_offset * y_max,
                "Jan" if month == 1 else "Jul",
                ha="center",
                va="top",
            )
...
if __name__ == "__main__":
    if not os.path.exists(CSV_PATH):
        ...
        # for i in range(1, 5):
        #     generate_bar_graph_years(data, dates, i, save=True)
